...
export const useRequestFlow = (requestId: string | null) => {
  return useQuery({
    queryKey: ['financial-request-flow', requestId],
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          billed_invoice:invoices!financial_requests_billed_invoice_id_fkey(id, code, status, total_amount),
          liquidation:liquidations!financial_requests_liquidation_id_fkey(id, code, status, total_amount)
        `)
        .eq('id', requestId)
        .single();
...
export const useRequestsWithFlow = (filters?: any) => {
  return useQuery({
    queryKey: ['financial-requests-with-flow', filters],
    queryFn: async () => {
      let query = supabase
        .from('financial_requests')
        .select(`
