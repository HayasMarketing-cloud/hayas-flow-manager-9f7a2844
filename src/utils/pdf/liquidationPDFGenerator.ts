...
const ensureConsistentView = (view: LiquidationView, expectedTotal?: number | string | null): LiquidationView => {
  ...
  if (Number.isFinite(expected) && Math.abs(expected - view.grandTotal) > 0.005) {
    throw new Error(
      `El PDF no puede generarse porque el subtotal de la liquidación (${formatCurrency(expected)}) no coincide con el total de items (${formatCurrency(view.grandTotal)}).`
    );
  }
